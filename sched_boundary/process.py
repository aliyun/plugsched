...
def lookup_if_global((name, file)):
    ...
    return (name, global_fn_dict.get(name, None) if file == '?' else file)
...
if __name__ == '__main__':
    ...
    for meta in metas:
        for fn_ptr in meta['fn_ptr']:
            if fn_ptr[1] in config['mod_files']:
                fn_symbol_classify['fn_ptr'].add(lookup_if_global(fn_ptr))

        for edge in meta['edge']:
            (from_n, from_f) = edge['from']
            edge['to'] = lookup_if_global(edge['to'])
            if edge['to']:    # edge_to might be gcc builtin functions
                edges.append(edge)
